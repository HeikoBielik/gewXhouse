...
model StandardHouse "Definition of the house dimensions and orientation"
  ...
  Modelica.SIunits.Area totalSurface "total surface";
  Modelica.SIunits.Volume volumeHouse "";
  Real surfacesA[N];
  ...
  Models.Air air(T(start = 293.15)) annotation(
    Placement(visible = true, transformation(origin = {20, -20}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
...
equation
  ...
  connect(environment, cover.environment) annotation(
    Line(points = {{110, -20}, {74, -20}, {74, -20}, {74, -20}}));
  connect(air.heatPort, cover.heatPort) annotation(
    Line(points = {{20, -10}, {20, 10}, {60, 10}, {60, -20}}, color = {191, 0, 0}));
  connect(floor.heatPort, air.heatPort) annotation(
    Line(points = {{-20, -20}, {-20, 10}, {20, 10}, {20, -10}}, color = {191, 0, 0}));
  connect(temperatureSensor.port, cover.heatPort) annotation(
    Line(points = {{80, 10}, {60, 10}, {60, -20}}, color = {191, 0, 0}));
  ...
  floor.surface = floorSurface.A;
  air.volumeHouse = volumeHouse;
  cover.totalSurface = totalSurface;
  posHouse[1] = lat;
  posHouse[2] = long;
  annotation(
