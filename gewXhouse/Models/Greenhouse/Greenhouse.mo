...
equation
 ...
 g.I= Radiation.I_dir * (cos(g.a * K) + cos(abs(Radiation.azimuth * K - (g.o + o) * K)) * sin(g.a * K) * tan(Radiation.elevation * K));
  ...
  Wall.Q        = S * Wall.c * (environment.Tout - T_air) / Wall.w; //flow wall -> air
  C_in          = m * cp; //capa air
  ...
  g.Qw = g.Rue*g.S*(g.T - T_air); //flow floor -> air
  
  Qv=dl*cp*nl*V*(environment.Tout-T_air)*V_on_off;
