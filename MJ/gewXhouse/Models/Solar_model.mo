...
model Solar_model 
  ...
  Connectors.Interfaces.HeatFluxInput  I_glob annotation(
    Placement(visible = true, transformation(extent = {{-140, -20}, {-100, 20}}, rotation = 0), iconTransformation(origin = {-110, 60}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Connectors.Interfaces.House house annotation(
  ...
  Connectors.Interfaces.SunPos sunPos annotation(
...
algorithm
  for i in 1:house.N loop
    S := house.surfaces[i];
    S.I := I_glob * (cos(S.a) + cos(abs(sunPos.azimuth - S.o)) * sin(S.a) * tan(sunPos.elevation));
    S.Ip := if S.I < 0 or sunPos.elevation < 0 or (abs(S.a-sunPos.elevation)) > 41 then 0 else S.I;
    S.Q := S.Ip * S.A;
    I_Intern := I_Intern+S.Ip;
    house.surfaces[i] := S;
  end for;
